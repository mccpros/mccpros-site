...
class HeroPage extends Component {
  ...
  componentWillReceiveProps(newProps) {
    const heroId = newProps.match.params.id;
    if (
      newProps.heroes &&
      (!this.state.selectedHero.content_id || this.state.selectedHero.content_id !== heroId)
    ) {
      newProps.heroes.forEach(h => {
        if (h.content_id == heroId && this.state.selectedHero.content_id !== h.content_id) {
  ...
  renderHero() {
    ...
    let stats = [hero.stats_1, hero.stats_2, hero.stats_3, hero.stats_4];
        ...
        <div className="container-fluid no-padding">
          <div className="row background">
            <div className="col-xs-12 col-sm-offset-6 col-sm-6">
              <div>
                <h1 className="arvo white">{hero.name}</h1>
                <p className="arvo green hero-title">{hero.title}</p>
              </div>
            </div>
            ...
            <div className="row banner">
              <div className="col-xs-12 col-md-6">
                <div className="col-xs-2 col-sm-6">
                  {this.isMobile ? (
                    <img className="mobile-hero-img" src={hero.full_superhero} alt="IT Superhero" />
                  ) : (
                    <Parallax
                ...
                <div className="col-xs-10 col-sm-6">
                  <div className="stats-wrapper">
                    ...
                    {stats.map((s, idx) => {
                      if (!s || s === undefined) return <div key={idx} />;
                      let label = s.split('|')[0];
              ...
              <div className="col-xs-12 col-sm-6 col-sm-offset-6 col-md-offset-0 col-md-6 relative">
                <div className="hero-info">
                  ...
                  <p className="white lato">{hero.expertise}</p>
                </div>
                <div className="hero-info">
                  <p className="hero-label lato green">CERTIFIED:</p>
                  <p className="white lato">{hero.certified}</p>
                </div>
                <div className="hero-info">
                  <p className="hero-label lato green">EXPERIENCE:</p>
                  <p className="white lato">{hero.experience}</p>
                </div>
              </div>
            </div>
          </div>
  ...
  render() {
    return (
      <div style={{ opacity: this.state.show ? 1 : 0 }} className="page-container">
        <NavContainer {...this.props} />

        <div id="pageWrapper">
          <div className="page-parent">
            {this.state.selectedHero.content_id ? (
              this.renderHero()
            ) : (
              <TransitionWrapper>
                <Loader />
